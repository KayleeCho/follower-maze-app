...
case class Private(override val seqNo: Long,  from: Long, to: Long, override val payload: String) extends Message
...
object Message {
  def apply(payload: String): Either[ParseError, Message] =
    payload.split("\\|").toList match {
      ...
      case List(Converter(id), "P", Converter(from), Converter(to)) => Right(Private(id, from, to, payload))
